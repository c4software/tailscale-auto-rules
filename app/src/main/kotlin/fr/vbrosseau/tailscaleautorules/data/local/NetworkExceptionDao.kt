...
interface NetworkExceptionDao {
    ...
    fun observeAll(): Flow<List<NetworkExceptionEntity>>

    @Query("SELECT * FROM network_exception")
    suspend fun getAll(): List<NetworkExceptionEntity>

    @Query("SELECT * FROM network_exception WHERE network_key = :networkKey")
    suspend fun findByKey(networkKey: String): NetworkExceptionEntity?

    @Insert
    suspend fun insert(entity: NetworkExceptionEntity)

    @Update
    suspend fun update(entity: NetworkExceptionEntity)
     ...
     */
    @Transaction
    suspend fun upsertByKey(entity: NetworkExceptionEntity) {
